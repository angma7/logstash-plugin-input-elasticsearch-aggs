...
class LogStash::Inputs::Elasticsearch < LogStash::Inputs::Base
  ...
  def run(output_queue)
    ...
    Stud.interval(@interval) do
      ...
      countr = @client.count(@countoption)
      currentcount = countr['count']
      ...
      if currentcount > previouscount
        ...
        if @scan
          r = process_next_aggs_scroll(output_queue, r['_scroll_id'])
          has_hits = r['has_hits']
        else # not a scan, process the response
            aggregations = r['aggregations']['terms_keyword']['buckets']
            push_aggs(aggregations, output_queue)
            # aggregations.each { |hit| push_aggs(hit, output_queue) }
  ...
  private
  def process_next_aggs_scroll(output_queue, scroll_id)
    r = scroll_request(scroll_id)
    r['hits']['hits'].each { |hit| push_aggs(r['aggregations']['terms_keyword']['buckets'], output_queue) }
    {'has_hits' => r['hits']['hits'].any?, '_scroll_id' => r['_scroll_id']}
  end
  ...
  def push_aggs(aggregations, output_queue)
    aggregations.each do |buckets|
        event = LogStash::Event.new(buckets)
        #event = LogStash::Event.new(aggregations['buckets'])
        ...
        event[@docinfo_target] ||= {}
        #unless event[@docinfo_target].is_a?(Hash)
        #  @logger.error("Elasticsearch Input: Incompatible Event, incompatible type for the `@metadata` field in the `buckets` document, expected a hash got:", :metadata_type => event[@docinfo_target].class)

        #  raise Exception.new("Elasticsearch input: incompatible event")
        #end
        event[@docinfo_target]['keyword'] = buckets['key']
        event[@docinfo_target]['rankings'] = buckets['key_score']['buckets']

        #@docinfo_fields.each do |field|
        #  event[@docinfo_target][field] = buckets[field]
        #end
      # end
      output_queue << event
    end
